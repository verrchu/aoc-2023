...
fn main() {
    ...
    let extract_surrounding_numbers = |schema: &mut Vec<Vec<char>>, r: usize, c: usize| -> usize {
        let mut sum = 0;

        let r = r as isize;
        let c = c as isize;

        extract_number(schema, r - 1, c - 1).map(|n| sum += n);
        extract_number(schema, r - 1, c).map(|n| sum += n);
        extract_number(schema, r - 1, c + 1).map(|n| sum += n);
        extract_number(schema, r, c - 1).map(|n| sum += n);
        extract_number(schema, r, c + 1).map(|n| sum += n);
        extract_number(schema, r + 1, c - 1).map(|n| sum += n);
        extract_number(schema, r + 1, c).map(|n| sum += n);
        extract_number(schema, r + 1, c + 1).map(|n| sum += n);
    ...
    for r in 0..nrows {
        for c in 0..ncols {
            let val = schema[r][c];
            if !char::is_digit(val, 10) && val != '.' {
                result += extract_surrounding_numbers(&mut schema, r, c);
            }
        }
    }
